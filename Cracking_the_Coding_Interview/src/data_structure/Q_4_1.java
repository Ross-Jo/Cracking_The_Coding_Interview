...
public class Q_4_1 {
	...
	public static Node_4_1 treeCreate(Node_4_1 root, int num) {
		...
		while (!q.isEmpty()) {
			Node_4_1 t = q.poll();
			if (check != num) {
				Node_4_1 lc = new Node_4_1(++name, t, t.hight + 1);
				check++;
				t.leftChildren = lc;
			...
			if (check != num) {
				Node_4_1 rc = new Node_4_1(++name, t, t.hight + 1);
				check++;
				t.RightChildren = rc;
				System.out.println("t.name: " + t.name + " ");
	...
	public static void BBTsearch(Node_4_1 root) {
		...
		visit(root);
		root.visited = true;
		if (root.leftChildren != null && root.visited == false)
			BBTsearch(root.leftChildren);
		if (root.RightChildren != null && root.visited == false)
			BBTsearch(root.RightChildren);
	}
